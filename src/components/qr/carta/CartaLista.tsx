  ...
  return (
    <div className="min-h-screen font-[family-name:var(--font-dm)]" style={{ background: "#f7f7f5" }}>
      ...
      {typeof window !== "undefined" && (() => {
        ...
        if (msgType) return <div style={{ paddingTop: 8 }}><GenioDietMessage type={msgType} diet={diet} restrictions={activeRestrictions} restaurantName={restaurant.name} /></div>;
        if (!mode) return null;
        return (
      ...
      {grouped.map(({ category, dishes: catDishes }, index) => (
        <section key={category.id} id={`lista-cat-${category.id}`} style={{ padding: "12px 12px 0" }}>
