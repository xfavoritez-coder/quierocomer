...
/**
 * Guard del pilar Centro de pedidos. Solo se muestra a restaurantes que el
 * superadmin habilitó (centroPedidosEnabled). El sidebar ya oculta el menú,
 * esto protege el acceso directo por URL.
 */
export default function CentroPedidosLayout({ children }: { children: React.ReactNode }) {
  const session = useSessionContext();
  const selected = session?.restaurants.find((r) => r.id === session.selectedRestaurantId);
  const enabled = !!(selected as any)?.centroPedidosEnabled;
