...
export function buildPrintAgentInstaller(token: string, baseUrl: string, label?: string): string {
  ...
  const csharp = buildAgentCSharp(token, baseUrl, label, slug);
...
function buildAgentCSharp(token: string, baseUrl: string, label?: string, slug?: string): string {
  const t = token.replace(/["\\]/g, "");
  const b = baseUrl.replace(/["\\]/g, "");
  const sg = (slug || "").replace(/[^a-zA-Z0-9_]/g, "");
...
namespace QuieroComerAgente {
  ...
  public class TrayApp : ApplicationContext {
    ...
    const string SLUG    = "${sg}";  // sufijo unico de esta instancia (ej: "_abc123")
    const string LABEL   = "${lbl}"; // nombre del local (para avisos)
    ...
    public TrayApp() {
      ...
      menu.Items.Add("Desinstalar este agente", null, delegate { Uninstall(); });
    ...
    // Auto-desinstala SOLO esta instancia (este local): quita el arranque, agenda el
    // borrado de la carpeta (no se puede borrar en uso) y cierra el agente.
    void Uninstall() {
      string quien = string.IsNullOrEmpty(LABEL) ? "este local" : LABEL;
      if (MessageBox.Show("Desinstalar el agente de impresion de " + quien + "?\n\nDejara de imprimir en este equipo. Puedes volver a instalarlo desde el panel.", "Agente de impresion quierocomer", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes) return;
      try {
        // Quitar del arranque de Windows
        try { RegistryKey rk = Registry.CurrentUser.OpenSubKey("Software\\Microsoft\\Windows\\CurrentVersion\\Run", true); if (rk != null) { rk.DeleteValue("QuieroComerAgente" + SLUG, false); rk.Close(); } } catch {}
        // Borrar carpeta + matar este proceso desde un cmd desacoplado (espera 2s)
        string dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "QuieroComerAgente" + SLUG);
        string args = "/c timeout /t 2 >nul & taskkill /IM AgenteImpresion" + SLUG + ".exe /F >nul 2>&1 & rmdir /s /q \"" + dir + "\"";
