...
import { computeLifecycleStage, computeEngagementScore } from "@/lib/admin/lifecycle";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const entries = restaurants.map(r => {
      ...
      return {
        id: r.id, slug: r.slug, name: r.name, logoUrl: r.logoUrl,
        plan: r.plan, stage, engagement, engagementChecks, salud, trialDaysLeft,
    ...
    for (const lead of leads) {
      ...
      entries.push({
        id: `lead-${lead.id}`, slug: lead.generatedSlug,
        name: lead.localName || "Sin nombre", logoUrl: null,
        plan: "FREE" as any, stage, engagement, engagementChecks,
        salud: "gray", trialDaysLeft: null, lastActivity: null,
        sessions7d: 0, totalSessions: 0, dishes: 0, categories: 0,
        createdAt: lead.createdAt.toISOString(), owner: null,
        leadId: lead.id, leadOwner: lead.ownerName,
        leadEmail: lead.email, leadWhatsapp: lead.whatsapp,
        cartaOriginalUrl: lead.cartaFileUrl || lead.cartaUrl || null,
        cartaType: lead.cartaType || null,
    ...
    await prisma.systemCache.upsert({
      where: { key: "lifecycle" },
      update: { value: { entries, stats, computedAt: now.toISOString() } },
      create: { key: "lifecycle", value: { entries, stats, computedAt: now.toISOString() } },
    });

    return NextResponse.json({ ok: true, entries: entries.length, computedAt: now.toISOString() });
  } catch (error) {
    console.error("[lifecycle-snapshot cron]", error);
