      ...
      <div className="dish-modal">
        <div style={{ maxWidth: 460, margin: '0 auto' }}>
          ...
          <div style={{ position: 'relative', width: '100%', aspectRatio: '4/3', overflow: 'hidden', background: '#1a1a1a' }}>
            ...
            {/* Category */}
            <span className="dish-card-badge" style={{ top: 16, bottom: 'auto', left: 16 }}>
              {dish.categoriaNorm}
            </span>
            ...
            {/* Oferta */}
            {dish.enOferta && (
              <span className="dish-card-oferta" style={{ top: 16 }}>En oferta</span>
            )}
          </div>

          {/* Content */}
          <div style={{ padding: '0 20px 32px', marginTop: -12, position: 'relative' }}>
            ...
            {/* Reason */}
            {reason && (
              <p style={{ color: 'rgba(244,166,35,0.6)', fontSize: 12, fontStyle: 'italic', margin: '0 0 12px' }}>
                {reason}
              </p>
            )}
                  ...
                  return (
                    <button key={star} onClick={() => { setUserRating(star); onRate(dish, star) }}
                      ...
                      <svg width="22" height="22" viewBox="0 0 24 24"
                        ...
                        stroke={filled ? '#F4A623' : 'rgba(255,255,255,0.15)'}
                        strokeWidth="2">
            ...
            <a href={`/qr/${dish.restauranteSlug}`} target="_blank" rel="noopener noreferrer"
              ...
              {dish.restauranteLogo ? (
                <img src={dish.restauranteLogo} alt="" style={{ width: 40, height: 40, borderRadius: '50%', objectFit: 'cover', background: 'rgba(255,255,255,0.1)' }} />
              ) : (
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 14, fontWeight: 600, color: '#fff', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{dish.restaurante}</p>
                {dish.restauranteDireccion && (
                  <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', margin: '2px 0 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{dish.restauranteDireccion}</p>
                )}
              </div>
              <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', flexShrink: 0 }}>Ver carta →</span>
            </a>

            {/* Description */}
            {dish.descripcion && (
              <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)', lineHeight: 1.5, margin: '0 0 16px' }}>
                {dish.descripcion}
              </p>
            )}
            ...
            {(dish.dieta.tipo !== 'OMNIVORE' || dish.dieta.sinGluten || dish.dieta.sinLactosa || dish.dieta.esPicante) && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 16 }}>
                {dish.dieta.tipo === 'VEGAN' && <span style={badgeStyle('#22c55e')}>Vegano</span>}
                {dish.dieta.tipo === 'VEGETARIAN' && <span style={badgeStyle('#22c55e')}>Vegetariano</span>}
            ...
            {/* Action buttons */}
            <div style={{ display: 'flex', justifyContent: 'center', gap: 32, padding: '8px 0 16px' }}>
              <ActionBtn icon="pass" label="Paso" active={false} color="#fff" onClick={() => { onPass(dish); onClose() }} />
              <ActionBtn icon="heart" label="Me gusta" active={liked} color="#ef4444" onClick={() => { setLiked(!liked); onLike(dish) }} />
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <input type="text" placeholder="Escribe un comentario..."
                  style={{
                    ...
                    borderRadius: 12, padding: '10px 14px', fontSize: 14, color: '#fff', outline: 'none',
                  }} />
                <button style={{
                  padding: '10px 16px', borderRadius: 12,
                  background: 'rgba(244,166,35,0.15)', border: 'none',
                  color: '#F4A623', fontSize: 14, fontWeight: 600, cursor: 'pointer',
                }}>
                  Enviar
                </button>
              </div>
...
function ActionBtn({ icon, label, active, color, onClick }: {
  ...
  const iconSvg = {
    pass: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
      <path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zm7-13h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17" />
    </svg>,
    heart: <svg width="20" height="20" viewBox="0 0 24 24" fill={active ? color : 'none'} stroke={active ? color : 'currentColor'} strokeWidth="2" strokeLinecap="round">
      <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
    </svg>,
    save: <svg width="20" height="20" viewBox="0 0 24 24" fill={active ? color : 'none'} stroke={active ? color : 'currentColor'} strokeWidth="2" strokeLinecap="round">
