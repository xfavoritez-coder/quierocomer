...
export default function GenioVegetarianCarousel({ dishes, categories, onDishClick }: Props) {
  const lang = useLang();

  const vegDishes = useMemo(() => {
    return dishes.filter(d => ((d as any).dishDiet === "VEGETARIAN" || (d as any).dishDiet === "VEGAN") && d.isActive);
  }, [dishes]);

  if (vegDishes.length === 0) return null;

  const title = (t(lang, "gVegetarianDishesForYou" as any) || `${vegDishes.length} opciones vegetarianas para ti 🥗`).replace("{n}", String(vegDishes.length));
  ...
  return (
    <div
      ...
      style={{
        ...
        background: "#F0F5E6",
        border: "0.5px solid rgba(107,142,35,0.2)",
        borderRadius: 16,
      }}
    >
      {/* Header */}
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <div style={{
          ...
          background: "#fff", border: "0.5px solid rgba(107,142,35,0.3)",
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: "0.5px solid rgba(107,142,35,0.15)",
              ...
              >
                <div style={{ position: "relative", width: "100%", height: 72, borderRadius: 8, overflow: "hidden", background: "#f0f0f0", marginBottom: 5 }}>
                  ...
                  <span style={{
                    ...
                    color: isVegan ? "#3A8E68" : "#6B8E23", fontSize: "10px", fontWeight: 600,
                    padding: "2px 6px", borderRadius: 4, letterSpacing: "0.3px",
                    display: "flex", alignItems: "center", gap: 3,
                  }}>
                    <span style={{ width: 6, height: 6, borderRadius: "50%", background: isVegan ? "#3A8E68" : "#6B8E23", flexShrink: 0 }} />
                    {isVegan ? "VEGAN" : "VEG"}
                  </span>
                </div>
                <p style={{
                  fontSize: "0.7rem", fontWeight: 600, color: "#2D4A0E",
                ...
                <p style={{
                  ...
                  fontSize: "12px", fontWeight: 500, color: "#5A7D2B", margin: 0,
                }}>
