...
export interface ComanderoPanelProps {
  accountId: string | null
  /** true = modo panel desktop (sin navigate, usa callbacks) */
  isPanel?: boolean
  onClose: () => void
  onBack: () => void  // en desktop: volver a cuenta panel; en mobile: volver a /pos/cuenta
}

export default function ComanderoPanel({
  accountId,
  isPanel = false,
  onClose,
  onBack,
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      // Después de enviar, volver a la cuenta (en panel desktop: onBack; en mobile: navegar)
      if (isPanel) {
        onBack()
      } else {
  ...
  }, [orderItems, accountId, isPanel, onBack, navigate])
