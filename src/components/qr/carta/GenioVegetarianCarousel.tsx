          ...
          {vegDishes.slice(0, 20).map(d => {
            ...
            return (
              <button key={d.id} onClick={() => onDishClick?.(d.id)} className="active:scale-[0.97] transition-transform"
                ...
                <p style={{ fontSize: "14px", fontWeight: 600, color: "var(--carta-genio-veg-title)", margin: "0 0 1px", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.name}</p>
                ...
                <p style={{ fontFamily: "var(--font-dm), system-ui, sans-serif", fontSize: "13px", fontWeight: 500, color: "var(--carta-genio-veg-sub)", margin: 0 }}>${d.price.toLocaleString("es-CL")}</p>
              </button>
            );
          })}
        </div>
