...
async function main() {
  ...
  for (const r of restaurants) {
    ...
    const tooNew8 = ageMs < TWELVE_HOURS;
    ...
    targets.push({
      ...
      willSend8: !already && !tooNew8,
    });
  }

  // Los que se GANAN con 12h vs 24h
  const gained = targets.filter(t => t.willSend8 && !t.willSend24);
  ...
  const willSend8 = targets.filter(t => t.willSend8);
  const willSend24 = targets.filter(t => t.willSend24);
  const stillTooNew = targets.filter(t => t.tooNew8 && !t.alreadySent);

  console.log("\n=== TOTAL ENVÍOS CON 12h ===\n");
  for (const t of willSend8) {
  ...
  console.log(`Con 8h enviaría: ${willSend8.length} (+${gained.length} nuevos)`);
  console.log(`Aún muy nuevos (<8h): ${stillTooNew.length}`);
