    ...
    <>
      ...
      {needsComidas && !isHome && pathname !== "/perfil" && (
        <div style={{ background: "linear-gradient(135deg, rgba(232,168,76,0.12), rgba(232,168,76,0.06))", borderBottom: "1px solid rgba(232,168,76,0.2)", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
          ...
          <span style={{ fontFamily: "var(--font-body)", fontSize: "0.82rem", color: "rgba(240,234,214,0.7)" }}>Completa tu perfil para recomendarte mejor</span>
          <a href="/perfil?completar=comidas" style={{ fontFamily: "var(--font-display)", fontSize: "0.7rem", color: "#FFD600", textDecoration: "none", border: "1px solid rgba(232,168,76,0.3)", borderRadius: 8, padding: "4px 12px", whiteSpace: "nowrap" }}>Completar →</a>
        </div>
      )}
          ...
          {mounted && (
            isLocalLoggedIn ? (
              <div className="dc-nav-user">
                <Link href={`/locales/${localSession?.slug || localSession?.id}`} style={{ width: "32px", height: "32px", borderRadius: "50%", background: localSession?.logoUrl ? "transparent" : "linear-gradient(135deg, #2a7a6f, #3db89e)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-display)", fontSize: "0.7rem", fontWeight: 700, color: "#fff", flexShrink: 0, textDecoration: "none", overflow: "hidden" }}>{localSession?.logoUrl ? <img src={localSession.logoUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "50%" }} /> : localInitials}</Link>
                <Link href={`/locales/${localSession?.slug || localSession?.id}`} className="dc-nav-username" style={{ textDecoration: "none" }}>{localName}</Link>
                <Link href="/panel" style={{ fontFamily: "var(--font-display)", fontSize: "0.75rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "#3db89e", textDecoration: "none", padding: "6px 14px", borderRadius: "20px", border: "1px solid rgba(61,184,158,0.4)", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: "5px" }}>🏪 Mi Panel</Link>
                      ...
                      {notifs.length === 0 ? (
                        ...
                        n.esVerificacion ? (
                          ...
                          <div key={n.id} onClick={() => { const d = (n as any).datos; const href = d?.link || (d?.concursoSlug ? `/concursos/${d.concursoSlug}${d?.abrirVerificarTel ? "?verificar=tel" : ""}` : null); if (href) window.location.href = href; setShowNotifs(false); }} style={{ padding: "10px 14px", borderBottom: "1px solid rgba(232,168,76,0.06)", cursor: "pointer", background: n.leida ? "transparent" : "rgba(232,168,76,0.04)" }}>
                            <p style={{ fontFamily: "var(--font-body)", fontSize: "0.82rem", color: "rgba(240,234,214,0.7)", lineHeight: 1.4, margin: 0 }}>{n.mensaje}</p>
                            <p style={{ fontFamily: "var(--font-body)", fontSize: "0.68rem", color: "rgba(240,234,214,0.2)", marginTop: 4 }}>{new Date(n.createdAt).toLocaleDateString("es-CL", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}</p>
                          </div>
                        )
                      ))}
                    </div>
          ...
          {mounted && isLocalLoggedIn && (
            <Link href="/panel" style={{ textDecoration: "none", width: 30, height: 30, borderRadius: "50%", border: "1px solid rgba(61,184,158,0.4)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", flexShrink: 0, background: localSession?.logoUrl ? "transparent" : "linear-gradient(135deg, #2a7a6f, #3db89e)" }}>
              {localSession?.logoUrl ? (
                ...
                <span style={{ fontFamily: "var(--font-display)", fontSize: "0.6rem", fontWeight: 700, color: "#fff" }}>{localInitials}</span>
              )}
            </Link>
          )}
        ...
        <>
          ...
          <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "min(320px, 80vw)", background: "rgba(13,7,3,0.98)", borderLeft: "1px solid rgba(232,168,76,0.15)", zIndex: 1001, display: "flex", flexDirection: "column", animation: "dc-drawer-in 0.25s ease both", overflowY: "auto", WebkitOverflowScrolling: "touch" as any }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 24px", borderBottom: "1px solid rgba(232,168,76,0.08)" }}>
              <Link href="/" onClick={() => setMenuOpen(false)} style={{ fontFamily: "var(--font-display)", fontSize: "1rem", color: "var(--accent)", textDecoration: "none", display: "flex", alignItems: "center", gap: "8px" }}>🏮 QuieroComer</Link>
            ...
            <nav style={{ padding: "8px 0" }}>
              {[
                ...
                return (
                  <Link key={item.href} href={item.href} onClick={() => setMenuOpen(false)} style={{ display: "flex", alignItems: "center", gap: "14px", padding: "16px 24px", fontFamily: "var(--font-display)", fontSize: "1rem", letterSpacing: "0.1em", color: active ? "var(--accent)" : "var(--text-primary)", textDecoration: "none", borderBottom: "1px solid rgba(232,168,76,0.06)", background: active ? "rgba(232,168,76,0.06)" : "transparent", fontWeight: active ? 700 : 500 }}>
              ...
              {mounted && isLocalLoggedIn ? (
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "14px" }}>
                    <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: localSession?.logoUrl ? "transparent" : "linear-gradient(135deg, #2a7a6f, #3db89e)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-display)", fontSize: "0.9rem", fontWeight: 700, color: "#fff", flexShrink: 0, overflow: "hidden" }}>{localSession?.logoUrl ? <img src={localSession.logoUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "50%" }} /> : localInitials}</div>
                    <div><p style={{ fontFamily: "var(--font-display)", fontSize: "0.85rem", color: "#3db89e", margin: 0 }}>{localName}</p><p style={{ fontFamily: "var(--font-body)", fontSize: "0.7rem", color: "var(--text-muted)", margin: 0 }}>Local asociado</p></div>
                  </div>
                  {[
                    ...
                    <Link key={item.label} href={item.href} onClick={() => setMenuOpen(false)} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 0", fontFamily: "var(--font-body)", fontSize: "0.9rem", color: item.color, textDecoration: "none", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>{item.label}</Link>
                  ))}
                  <button onClick={handleLocalLogout} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 0", background: "none", border: "none", fontFamily: "var(--font-body)", fontSize: "0.9rem", color: "#ff6b6b", cursor: "pointer", width: "100%", textAlign: "left", marginTop: "4px" }}>Cerrar sesión</button>
                </div>
              ) : mounted && isAuthenticated && user ? (
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "14px" }}>
                    <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: "linear-gradient(135deg, #c4853a, #FFD600)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-display)", fontSize: "0.9rem", fontWeight: 700, color: "#0D0D0D", flexShrink: 0 }}>
                      {user.nombre?.charAt(0).toUpperCase() ?? "U"}
                    </div>
                    <div>
                      <p style={{ fontFamily: "var(--font-display)", fontSize: "0.85rem", color: "var(--accent)", margin: 0 }}>{displayName}</p>
                      <p style={{ fontFamily: "var(--font-body)", fontSize: "0.7rem", color: "var(--text-muted)", margin: 0 }}>Miembro de QuieroComer</p>
                    </div>
                  </div>
                  {[
                    ...
                    <Link key={item.label} href={item.href} onClick={() => setMenuOpen(false)} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 0", fontFamily: "var(--font-body)", fontSize: "0.9rem", color: "var(--text-primary)", textDecoration: "none", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                      <span style={{ width: "20px", display: "flex", alignItems: "center", justifyContent: "center" }}>{item.svg}</span>{item.label}
                    </Link>
                  ))}
                  <button onClick={() => { logout(); setMenuOpen(false); }} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 0", background: "none", border: "none", fontFamily: "var(--font-body)", fontSize: "0.9rem", color: "#ff6b6b", cursor: "pointer", width: "100%", textAlign: "left", marginTop: "4px" }}>
                ...
                <div>
                  <p style={{ fontFamily: "var(--font-body)", fontSize: "0.82rem", color: "var(--text-muted)", marginBottom: "12px", textAlign: "center" }}>Únete gratis o inicia sesión</p>
                  <Link href="/login" onClick={() => setMenuOpen(false)} style={{ display: "block", padding: "14px 20px", background: "var(--accent)", borderRadius: "12px", fontFamily: "var(--font-display)", fontSize: "0.85rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--bg-primary)", textDecoration: "none", textAlign: "center", fontWeight: 700 }}>Entrar</Link>
                </div>
              ) : null}
            </div>
          </div>
