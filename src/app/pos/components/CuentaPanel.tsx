...
export interface CuentaPanelProps {
  accountId: string
  fromTab?: string
  /** true = modo panel desktop (sin navigate, usa callbacks) */
  isPanel?: boolean
  onClose: () => void
  onGoToComandero: (accountId: string) => void
}
...
export default function CuentaPanel({
  ...
  const handleConfirmVoidAccount = useCallback(async () => {
    ...
    try {
      ...
      // En panel desktop cerramos el panel; en mobile navegamos atrás
      if (isPanel) {
        onClose()
      } else {
  ...
  }, [accountId, voidAccountReason, navigate, isPanel, onClose, fromTab])
  ...
  return (
    <div className="pos-shell">
      <PosHeader
        ...
        syncing={syncing}
        onBack={isPanel ? undefined : onClose}
        rightSlot={(
          <>
            {!isClosed && (
              <button
