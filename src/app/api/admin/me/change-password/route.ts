...
import { checkAdminAuth, getAdminId, isSuperAdmin } from "@/lib/adminAuth";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!adminId || isSuperAdmin(req)) {
      return NextResponse.json({ error: "Solo owners pueden cambiar su contraseña aquí" }, { status: 400 });
    }
