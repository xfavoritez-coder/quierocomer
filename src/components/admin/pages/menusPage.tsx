...
export default function AdminMenus() {
  ...
  const [eLeafOverride, setELeafOverride] = useState("");
  ...
  const startEditDish = async (d: Dish) => {
    ...
    setELeafOverride((d as any).leafOverride || "");
  ...
  const saveDishEdit = async () => {
    ...
    const updates: Record<string, any> = {
      ...
      leafOverride: eLeafOverride || null,
        ...
        <div style={{ padding: 24 }}>
          {!editMode ? (
            ...
            <>
              ...
              {/* Ingredientes y alérgenos derivados — quitados temporalmente: simplificación del editor */}

              {/* Categoría del feed (leaf override) */}
              <div style={{ marginBottom: 24 }}>
                <label style={LBL}>Categoría en el feed QuieroComer</label>
                <p style={{ fontFamily: F, fontSize: "0.75rem", color: "var(--adm-text3)", margin: "0 0 8px", lineHeight: 1.45 }}>
                  Si la categoría de tu carta es genérica (Especiales, Combos, Rincón Verde…), puedes indicar qué tipo de plato es para que aparezca en el lugar correcto del feed.
                </p>
                <select
                  value={eLeafOverride}
                  onChange={e => setELeafOverride(e.target.value)}
                  style={{ ...INP, color: eLeafOverride ? "var(--adm-text)" : "var(--adm-text3)" }}
                >
                  <option value="">— Heredar de la sección —</option>
                  ...
                  <optgroup label="Especialidades">
                    ...
                    <option value="Pollo y alitas">Pollo y alitas</option>
                  ...
                  <optgroup label="Entradas y fondos">
                    <option value="Entradas">Entradas</option>
                    <option value="Platos de fondo">Platos de fondo</option>
                  </optgroup>
                  <optgroup label="Saludable">
                    <option value="Ensaladas">Ensaladas</option>
                    <option value="Bowls">Bowls</option>
                    <option value="Saludable">Saludable</option>
                  </optgroup>
                  <optgroup label="Desayunos y café">
                    <option value="Desayunos">Desayunos</option>
                    <option value="Cafetería">Cafetería</option>
                    <option value="Amasandería">Amasandería</option>
                  </optgroup>
                  <optgroup label="Dulce y bebidas">
                ...
                {eLeafOverride && (
                  <button
                    onClick={() => setELeafOverride("")}
                    style={{ marginTop: 6, fontSize: "0.7rem", color: "var(--adm-text3)", background: "none", border: "none", cursor: "pointer", padding: 0, fontFamily: F }}
                  >
                    × Quitar (heredar de la sección)
                  </button>
                )}
              </div>
