...
export async function GET(request: Request) {
  try {
    ...
    // Each Session row counts as a visit — the sessionTracker already
    // dedupes refreshes within a 2-min window into the same Session, so
    // distinct DB rows really do represent distinct visits (refresh after
    // a short break, return later same day, return another day, etc).
