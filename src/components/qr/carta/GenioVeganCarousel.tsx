...
export default function GenioVeganCarousel({ dishes, categories, onDishClick, alsoGlutenFree }: Props) {
  ...
  const countText = String(veganDishes.length);
  const title = alsoGlutenFree
    ? `${countText} opciones veganas sin gluten 🌿`
    : t(lang, "gVeganDishesForYou").replace("{n}", countText);
          ...
          {veganDishes.slice(0, 20).map(d => {
            ...
            return (
              <button key={d.id} onClick={() => onDishClick?.(d.id)} className="active:scale-[0.97] transition-transform"
                ...
                <p style={{ fontSize: "0.7rem", fontWeight: 600, color: "#173404", margin: "0 0 2px", minHeight: 30, lineHeight: 1.35, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as any, overflow: "hidden" }}>{d.name}</p>
