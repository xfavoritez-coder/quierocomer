...
export default function DescubrirClient() {
  ...
  const fetchRecommendations = useCallback(async (
    ...
    try {
      ...
      const res = await fetch('/api/eureka', {
        ...
        body: JSON.stringify({ dishIds, dislikedIds: dislikedIds ?? [], lat: loc.lat, lng: loc.lng, maxKm, diet, restaurantId: restaurantId ?? undefined }),
      })
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Leer dislikes para enviar al API
        let dislikedIds: string[] = []
        try {
          const disRaw = localStorage.getItem('qc_eureka_disliked')
          if (disRaw) dislikedIds = (JSON.parse(disRaw) as FeedDish[]).map(d => d.id)
        } catch {}

        await fetchRecommendations(likedDishes.map(d => d.id), loc, initFilters, false, scoped, dislikedIds)
      } catch {}
      setLoading(false)
  ...
  const cappedRecommended = (() => {
    const results = allRecommended.slice(0, 10)
                          ...
                          {d.googleRating != null && (
                            <span style={{ flexShrink: 0, display: 'inline-flex', alignItems: 'center', gap: 2 }}>
                              ...
                              {d.googleRatingCount != null && (
                                <>
                                  <svg width="8" height="8" viewBox="0 0 24 24" fill="rgba(255,255,255,0.65)" stroke="none" style={{ marginLeft: 1 }}>
                        ...
                        <p style={{ margin: '2px 0 0', fontSize: 11, color: 'rgba(255,255,255,0.6)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 3 }}>
                          ...
                          {d.googleRating != null && (
                            <span style={{ flexShrink: 0, display: 'inline-flex', alignItems: 'center', gap: 1 }}>
                              ...
                              {d.googleRatingCount != null && (
                                <>
                                  <svg width="7" height="7" viewBox="0 0 24 24" fill="rgba(255,255,255,0.65)" stroke="none" style={{ marginLeft: 1 }}>
                                    <path d="M12 3C6.5 3 2 6.58 2 11c0 2.5 1.25 4.73 3.2 6.2L3 21l4.3-1.8c1.5.5 3.1.8 4.7.8 5.5 0 10-3.58 10-8S17.5 3 12 3z"/>
                                  </svg>
