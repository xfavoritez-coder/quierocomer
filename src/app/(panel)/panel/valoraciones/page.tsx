...
export default function ValoracionesPage() {
    ...
    try {
      const res = await fetch("/api/panel/valoraciones", {
        ...
        body: JSON.stringify({
          restaurantId: rid,
          reviewMode: mode,
          googleReviewUrl: mode === "google" ? googleUrl.trim() || null : null,
  ...
  const previewLabel = hasReward && reward.trim() ? "Comenta y gana" : "Déjanos tu opinión";
  ...
  const showPreview = mode === "private" || (mode === "google" && googleUrl.trim());
  ...
  return (
    <div style={{ padding: "24px 16px", maxWidth: 720, margin: "0 auto", fontFamily: FB }}>
      ...
      {/* Google URL — solo si modo google */}
      {mode === "google" && (
        <div style={CARD}>
