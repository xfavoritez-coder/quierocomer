...
export default function GenioVegetarianCarousel({ dishes, categories, onDishClick, alsoGlutenFree }: Props) {
  ...
  return (
    <div
      ...
      style={{ margin: "0 12px 10px", padding: "14px 12px 12px", background: "var(--carta-genio-veg-bg)", border: "0.5px solid var(--carta-genio-veg-border)", borderRadius: 16 }}
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <div style={{ width: 36, height: 36, borderRadius: "50%", background: "var(--carta-surface)", border: "0.5px solid var(--carta-genio-veg-border2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.1rem", flexShrink: 0 }}>🧞</div>
        <div>
          <p style={{ fontSize: "0.94rem", fontWeight: 600, color: "var(--carta-genio-veg-title)", margin: 0 }}>{title}</p>
          <p style={{ fontSize: "0.73rem", color: "var(--carta-genio-veg-sub)", margin: "1px 0 0" }}>{subtitle}</p>
        </div>
      </div>
          ...
          {vegDishes.slice(0, 20).map(d => {
            ...
            return (
              <button key={d.id} onClick={() => onDishClick?.(d.id)} className="active:scale-[0.97] transition-transform"
                style={{ flexShrink: 0, width: 130, background: "var(--carta-surface)", border: "0.5px solid var(--carta-genio-veg-border3)", borderRadius: 12, padding: 6, cursor: "pointer", textAlign: "left", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
                <div style={{ position: "relative", width: "100%", height: 72, borderRadius: 8, overflow: "hidden", background: "var(--carta-img-placeholder)", marginBottom: 5 }}>
                  ...
                  <span style={{ position: "absolute", top: 6, left: 6, background: "var(--carta-genio-veg-tag-bg)", color: isVegan ? "var(--carta-genio-veg-tag)" : "var(--carta-genio-veg-tag2)", fontSize: "10px", fontWeight: 600, padding: "2px 6px", borderRadius: 4, letterSpacing: "0.3px", display: "flex", alignItems: "center", gap: 3 }}>
                    <span style={{ width: 6, height: 6, borderRadius: "50%", background: isVegan ? "var(--carta-genio-veg-tag)" : "var(--carta-genio-veg-tag2)", flexShrink: 0 }} />{isVegan ? "VEGAN" : "VEG"}
                  </span>
                </div>
                <p style={{ fontSize: "0.80rem", fontWeight: 600, color: "var(--carta-genio-veg-title)", margin: "0 0 1px", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.name}</p>
                {d.description && <p style={{ fontSize: "0.63rem", color: "var(--carta-genio-veg-desc)", margin: "0 0 2px", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.description}</p>}
                <p style={{ fontFamily: "var(--font-dm), system-ui, sans-serif", fontSize: "12px", fontWeight: 500, color: "var(--carta-genio-veg-sub)", margin: 0 }}>${d.price.toLocaleString("es-CL")}</p>
              </button>
            );
          })}
        </div>
        <div style={{ position: "absolute", top: 0, right: 0, bottom: 4, width: 32, background: "linear-gradient(to right, transparent, var(--carta-genio-veg-bg))", pointerEvents: "none", borderRadius: "0 12px 12px 0" }} />
      </div>
