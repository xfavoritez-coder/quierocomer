...
export default function ResetPasswordPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><p style={{ color: "#F4A623", fontSize: "1.2rem" }}>🧞</p></div>}>
      <ResetPasswordForm />
    </Suspense>
  );
}
...
function ResetPasswordForm() {
  ...
  const validate = (): string | null => {
    ...
    if (!/\d/.test(password)) return "La contraseña debe contener al menos 1 número";
    ...
    return (
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 2, textAlign: "center" }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.95)", borderRadius: 12, padding: "32px 24px", maxWidth: 320, border: "0.5px solid rgba(244,166,35,0.5)" }}>
        ...
        {success ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div>
                <label htmlFor="rp-pass" style={labelStyle}>Nueva contraseña</label>
                <input
                  id="rp-pass"
                  type="password"
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2, marginTop: -4 }}>
                <span style={{ fontFamily: "var(--font-display)", fontSize: "0.7rem", color: password.length >= 8 ? "#16a34a" : "#aaa" }}>
                  {password.length >= 8 ? "✓" : "○"} Mínimo 8 caracteres
                </span>
                <span style={{ fontFamily: "var(--font-display)", fontSize: "0.7rem", color: /\d/.test(password) ? "#16a34a" : "#aaa" }}>
                  {/\d/.test(password) ? "✓" : "○"} Al menos 1 número
                </span>
                <span style={{ fontFamily: "var(--font-display)", fontSize: "0.7rem", color: password && password === confirm ? "#16a34a" : "#aaa" }}>
                  {password && password === confirm ? "✓" : "○"} Contraseñas coinciden
                </span>
              </div>
