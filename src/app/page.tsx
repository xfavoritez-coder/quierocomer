  ...
  if (phase === "onboarding") {
    return (
      <div style={{ minHeight: "100vh", background: "#0D0D0D", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px 20px" }}>
        ...
        <p style={{ fontFamily: "var(--font-body)", fontSize: "0.9rem", color: "#888888", textAlign: "center", marginBottom: 32, maxWidth: 400 }}>3 preguntas rapidas para recomendarte mejor.</p>
          ...
          <div style={{ width: "100%", maxWidth: 400 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {DIET_TYPES.map(d => {
                ...
                return (
                  <button key={d.v} onClick={() => { setDietType(d.v); setTimeout(() => setObStep(1), 200); }} style={{ padding: "16px 18px", background: active ? "rgba(255,214,0,0.12)" : "#1A1A1A", border: active ? "1px solid #FFD600" : "1px solid #2A2A2A", borderRadius: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 12, textAlign: "left" }}>
        ...
        {obStep === 1 && (
          <div style={{ width: "100%", maxWidth: 400 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {ALLERGIES.map(a => {
                ...
                return (
                  ...
                  }} style={{ padding: "14px 18px", background: active ? "rgba(255,214,0,0.12)" : "#1A1A1A", border: active ? "1px solid #FFD600" : "1px solid #2A2A2A", borderRadius: 12, fontFamily: "var(--font-body)", fontSize: "0.92rem", color: active ? "#FFD600" : "#888888", cursor: "pointer", textAlign: "left" }}>
                    {active ? "✓ " : ""}{a.l}
                  </button>
                );
              })}
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 20 }}>
              <button onClick={() => setObStep(0)} style={{ flex: 1, padding: 14, background: "transparent", border: "1px solid #2A2A2A", borderRadius: 99, fontFamily: "var(--font-display)", fontSize: "0.85rem", color: "#FFFFFF", cursor: "pointer" }}>Atras</button>
          ...
          <div style={{ width: "100%", maxWidth: 400 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {FITNESS_OPTIONS.map(f => {
                ...
                return (
                  <button key={f.v} onClick={() => setFitnessMode(f.v)} style={{ padding: "14px 18px", background: active ? "rgba(255,214,0,0.12)" : "#1A1A1A", border: active ? "1px solid #FFD600" : "1px solid #2A2A2A", borderRadius: 12, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 12 }}>
                    <span style={{ fontSize: 22 }}>{f.emoji}</span>
                    <div>
                      <p style={{ fontFamily: "var(--font-display)", fontSize: "0.88rem", color: active ? "#FFD600" : "#FFFFFF", margin: 0 }}>{f.l}</p>
                      {f.sub && <p style={{ fontFamily: "var(--font-body)", fontSize: "0.75rem", color: "#888888", margin: 0 }}>{f.sub}</p>}
                    </div>
                  </button>
                );
              })}
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 20 }}>
              <button onClick={() => setObStep(1)} style={{ flex: 1, padding: 14, background: "transparent", border: "1px solid #2A2A2A", borderRadius: 99, fontFamily: "var(--font-display)", fontSize: "0.85rem", color: "#FFFFFF", cursor: "pointer" }}>Atras</button>
              <button onClick={saveOnboarding} disabled={savingOb} style={{ flex: 2, padding: 14, background: "#FFD600", color: "#0D0D0D", border: "none", borderRadius: 99, fontFamily: "var(--font-display)", fontSize: "0.9rem", fontWeight: 700, cursor: "pointer", opacity: savingOb ? 0.5 : 1 }}>{savingOb ? "Guardando..." : "Listo, recomiendame"}</button>
            </div>
          </div>
        )}
      </div>
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#0D0D0D", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
        ...
        <div style={{ width: "100%", maxWidth: 340, display: "flex", flexDirection: "column", gap: 10 }}>
          <button onClick={goSolo} style={{ padding: "18px 20px", background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 14, cursor: "pointer", display: "flex", alignItems: "center", gap: 14 }}>
            <span style={{ fontSize: 24 }}>🧑</span>
            <span style={{ fontFamily: "var(--font-display)", fontSize: "0.95rem", color: "#FFFFFF" }}>Voy solo</span>
          </button>
          <button onClick={goGroup} style={{ padding: "18px 20px", background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 14, cursor: "pointer", display: "flex", alignItems: "center", gap: 14 }}>
            <span style={{ fontSize: 24 }}>👥</span>
            <span style={{ fontFamily: "var(--font-display)", fontSize: "0.95rem", color: "#FFFFFF" }}>Estoy con alguien</span>
          </button>
        </div>
      </div>
    );
      ...
      <div style={{ minHeight: "100vh", background: "#0D0D0D", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
        ...
        <div style={{ display: "flex", gap: 12, width: "100%", maxWidth: 340 }}>
          {([["LOVED", "😍", "Me encanto"], ["MEH", "😐", "Regular"], ["DISLIKED", "😕", "No era lo mio"]] as const).map(([score, emoji, label]) => (
            <button key={score} onClick={() => submitFeedback(score)} style={{ flex: 1, padding: "18px 8px", background: "#1A1A1A", border: "1px solid #2A2A2A", borderRadius: 16, cursor: "pointer", textAlign: "center" }}>
              <span style={{ fontSize: 32, display: "block", marginBottom: 6 }}>{emoji}</span>
              <span style={{ fontFamily: "var(--font-display)", fontSize: "0.7rem", color: "#888888" }}>{label}</span>
            </button>
          ))}
        </div>

        <button onClick={() => { setPendingFeedback(null); setPhase("dishes"); }} style={{ marginTop: 16, padding: 10, background: "transparent", border: "none", fontFamily: "var(--font-body)", fontSize: "0.78rem", color: "#888888", cursor: "pointer" }}>
          Saltar
        </button>
      </div>
    );
  ...
  if (phase === "loading" || loadingDishes) {
    return (
      <div style={{ minHeight: "100vh", background: "#0D0D0D", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <p style={{ fontSize: 40, marginBottom: 12 }}>🧞</p>
        <p style={{ fontFamily: "var(--font-display)", fontSize: "0.9rem", color: "#888888" }}>Buscando platos...</p>
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100vh", background: "#0D0D0D", padding: "clamp(20px,4vw,40px) clamp(16px,3vw,24px)" }}>
      <div style={{ maxWidth: 500, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(1.2rem,3.5vw,1.6rem)", color: "#FFD600", marginBottom: 6 }}>Que te tinca?</h1>
          <p style={{ fontFamily: "var(--font-body)", fontSize: "0.85rem", color: "#888888" }}>Toca los platos que te llamen</p>
        </div>

        {dishes.length === 0 ? (
          <p style={{ fontFamily: "var(--font-body)", color: "#888888", textAlign: "center", padding: 40 }}>No hay platos disponibles aun. Vuelve pronto.</p>
        ) : (
          <>
            {/* Selected count */}
            {selected.size > 0 && (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 20 }}>
              {dishes.map((d: Dish) => {
                ...
                return (
                  <div key={d.id}
                    style={{ position: "relative", aspectRatio: "1", borderRadius: 14, overflow: "hidden", border: isSel ? "2px solid #3db89e" : "2px solid transparent", cursor: "pointer", background: "#1A1A1A" }}
            ...
            <button onClick={handleNext} disabled={selected.size === 0} style={{ width: "100%", padding: 16, background: selected.size > 0 ? "#FFD600" : "rgba(255,214,0,0.15)", color: selected.size > 0 ? "#0D0D0D" : "#888888", border: "none", borderRadius: 99, fontFamily: "var(--font-display)", fontSize: "0.92rem", fontWeight: 700, cursor: selected.size > 0 ? "pointer" : "default", marginBottom: 10 }}>
              {selected.size > 0 ? `Estos me tincan (${selected.size}) →` : "Selecciona al menos 1"}
            </button>

            <button onClick={handleOtherDishes} style={{ width: "100%", padding: 14, background: "transparent", border: "1px solid #2A2A2A", borderRadius: 99, fontFamily: "var(--font-display)", fontSize: "0.82rem", color: "#FFFFFF", cursor: "pointer" }}>
        ...
        {previewDish && (
          <>
            ...
            <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "min(90vw, 400px)", zIndex: 101, borderRadius: 20, overflow: "hidden", background: "#1A1A1A", border: "1px solid #2A2A2A" }}>
              ...
              <div style={{ padding: "16px 20px" }}>
                ...
                <p style={{ fontFamily: "var(--font-body)", fontSize: "0.82rem", color: "#888888", marginBottom: 8 }}>{previewDish.local?.nombre} · ${Number(previewDish.precio).toLocaleString("es-CL")}</p>
                {previewDish.ingredients?.length > 0 && (
                  <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 12 }}>
                    {previewDish.ingredients.map((ing: string) => (
                      <span key={ing} style={{ padding: "2px 8px", borderRadius: 99, background: "#222222", border: "1px solid #2A2A2A", fontFamily: "var(--font-body)", fontSize: "0.7rem", color: "#FFD600" }}>{ing}</span>
                    ))}
                  </div>
                )}
                {(previewDish.avgRating != null || previewDish.totalLoved > 0) && (
                  <p style={{ fontFamily: "var(--font-body)", fontSize: "0.78rem", color: "#888888", marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <button onClick={() => setPreviewDish(null)} style={{ padding: "12px 20px", background: "transparent", border: "1px solid #2A2A2A", borderRadius: 12, fontFamily: "var(--font-display)", fontSize: "0.82rem", color: "#888888", cursor: "pointer" }}>
                    Cerrar
                  </button>
                </div>
              </div>
