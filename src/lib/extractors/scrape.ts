...
async function callClaude(prompt: string, maxTokens = 32000, model = MODEL_FAST): Promise<string> {
...
export async function extractWithScraper(cartaUrl: string, providerName?: string | null, extractionConfig?: any): Promise<ExtractionResult> {
  ...
  // Gourmedia: fetch via Jina + raw HTML for photos (CDN images only visible in raw HTML)
  if (getDomain(menuUrl).includes("gour.media")) {
    console.log("[Scraper] Gourmedia detected — fetching Jina content + raw HTML for photos...");
    try {
      // Fetch Jina (text content) + raw HTML (photos) in parallel
      const [jinaContent, rawHtml] = await Promise.all([
        fetchPage(menuUrl, true),
        fetchWithTimeout(menuUrl, 10000).catch(() => ""),
      ]);
      ...
      const content = combined.length > cfgMaxChars * 2 ? combined.slice(0, cfgMaxChars * 2) : combined;
...
- SOLO JSON, sin texto adicional.`);
    ...
    } catch (e) { console.log("[Scraper] Gourmedia extraction failed, falling back to generic scraper"); }
  }
