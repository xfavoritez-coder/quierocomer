...
import { Home, UtensilsCrossed, Tag, Menu, ChevronRight, X, LogOut, Lock, Mail, BarChart3, Bell, ContactRound, UsersRound, Zap, Store, UserCog, Megaphone, CreditCard, Receipt, Settings, Sun, Moon } from "lucide-react";
  ...
  return (
    <div className={theme === "dark" ? "theme-dark" : "theme-light"} style={{ minHeight: "100vh", background: "var(--adm-bg)" }}>
          ...
          <div style={{ padding: "8px 20px", flex: 1 }}>
            ...
            {basePath === "/panel" && (
              <a href="/panel/suscripcion" onClick={closeAccount} style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "14px 0", background: "none", border: "none", borderBottom: "1px solid var(--adm-card-border)", cursor: "pointer", textAlign: "left", textDecoration: "none" }}>
                <CreditCard size={18} color={activePlan === "PREMIUM" ? "#a78bfa" : activePlan === "GOLD" ? "#F4A623" : "var(--adm-text2)"} />
      ...
      <style>{`
        ...
        /* Landscape on mobile: hide all chrome, full content */
        @media (orientation: landscape) and (max-height: 500px) {
          .owl-sidebar { display: none; }
          .owl-mobile-header { display: none; }
          .owl-bottom-nav { display: none; }
          .owl-main { margin-left: 0; padding: 12px 16px 12px; min-height: 100vh; }
        }
      `}</style>
    </div>
  );
