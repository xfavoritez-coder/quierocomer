...
async function main() {
  ...
  const existingNurturing = await prisma.panelActivity.findMany({
    ...
    select: { restaurantId: true, action: true },
  });
  ...
  for (const r of restaurants) {
    ...
    targets.push({
      ...
      stage, action: nurturing.action, alreadySent: already, tooNew,
      willSend: !already && !tooNew,
    });
  }

  console.log("\n=== LOCALES QUE RECIBIRÁN MENSAJE HOY (Camila nurturing) ===\n");
  const willSend = targets.filter((t: any) => t.willSend);
  for (const t of willSend) {
    console.log(`  ✅ ${t.name} (${t.slug}) | ${t.stage} → ${t.action} | ${t.owner} ${t.whatsapp}`);
  }

  console.log("\n=== YA ENVIADO ANTES (no se repite) ===\n");
  const alreadyList = targets.filter((t: any) => t.alreadySent);
  for (const t of alreadyList) {
    console.log(`  ⏭️  ${t.name} (${t.slug}) | ${t.stage} → ${t.action}`);
  }

  const tooNew = targets.filter((t: any) => t.tooNew && !t.alreadySent);
  if (tooNew.length) {
    console.log("\n=== MUY NUEVOS (<24h, se saltarán) ===\n");
    for (const t of tooNew) console.log(`  🕐 ${t.name} (${t.slug}) | ${t.stage}`);
  }

  console.log("\n--- Resumen ---");
  console.log(`Total califican por stage: ${targets.length}`);
  console.log(`Enviarán mensaje HOY: ${willSend.length}`);
  console.log(`Ya enviado antes: ${alreadyList.length}`);
  console.log(`Muy nuevos (<24h): ${tooNew.length}`);
