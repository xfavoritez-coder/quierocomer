/**
 * Reclasifica los platos de Sandwichería Milly Mar con la taxonomía actualizada.
 * Corrige vienesas que quedaron como sándwich → completo.
 * Uso: npx tsx scripts/reclasificar-milly-mar.ts
 */
...
async function main() {
  const restaurant = await prisma.restaurant.findFirst({
    where: { name: { contains: 'milly', mode: 'insensitive' } },
    select: { id: true, name: true, slug: true },
  })
  if (!restaurant) {
    console.error('No se encontró Milly Mar')
    process.exit(1)
  }
  ...
  // Mostrar vienesas
  console.log('\nVienesas:')
  for (const d of dishes.filter(d => d.name.toLowerCase().includes('vienesa') || d.name.toLowerCase().includes('completo'))) {
