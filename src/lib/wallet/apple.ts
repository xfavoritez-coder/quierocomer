...
export async function buildPkpass(member: MemberLike, program: ProgramLike, restaurantName: string, restaurantLogo?: string | null): Promise<Buffer> {
  ...
  // Geo-notificaciones via locations[] NO se usan: Apple Wallet las muestra como ítems
  // persistentes en la pantalla de bloqueo que el usuario debe descartar manualmente.
  // Las notificaciones por cercanía se envían desde el panel como push manual (changeMessage).
  ...
  const pass = {
    ...
    storeCard: {
      ...
      backFields: [
        ...
        // evitando "pass was unchanged" incluso con lag de PgBouncer o pushes consecutivos.
        { key: "_v", label: "", value: String(Date.now()) },
      ],
    },
  };
