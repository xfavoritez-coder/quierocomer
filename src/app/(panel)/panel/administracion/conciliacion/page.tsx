            ...
            <div>
              ...
              {remaining !== 0 && (
                <p style={{ fontFamily: FB, fontSize: "0.75rem", color: "#f59e0b", margin: "0 0 8px", background: "#fef3c722", border: "1px solid #fde68a", borderRadius: 6, padding: "5px 10px" }}>
                  ⚠ {remaining > 0 ? `Quedarán ${fmtClp(remaining)} sin asignar` : `Exceso de ${fmtClp(remaining)}`} — el movimiento quedará pendiente para cuadrar después
                </p>
              )}
              <button
                onClick={doSplit}
                disabled={saving || splits.filter(s => s.categoryId && parseInt(s.amount.replace(/\D/g, ""), 10) > 0).length < 2}
                style={{ padding: "8px 20px", borderRadius: 8, border: "none", background: "#F4A623", color: "#fff", fontFamily: F, fontSize: "0.85rem", fontWeight: 700, cursor: "pointer" }}
              >
                {saving ? "Guardando..." : remaining === 0 ? "Guardar split" : "Guardar parcial"}
              </button>
            </div>
          )}
        </div>
