...
async function getRestaurantLanding(slug: string) {
  const r = await prisma.restaurant.findFirst({
    where: { slug, OR: [{ isActive: true }, { isDemo: true }] },
    select: {
      id: true,
      slug: true,
      name: true,
...
function RestaurantLanding({ r }: { r: NonNullable<Awaited<ReturnType<typeof getRestaurantLanding>>> & { id: string } }) {
  ...
  return (
    ...
    <main style={{
      ...
      padding: '36px 20px 60px', fontFamily: 'system-ui, -apple-system, sans-serif',
      ...
      <div style={{ marginTop: 36, display: 'flex', flexDirection: 'column', gap: 10, width: '100%', maxWidth: 380 }}>
        ...
        {r.orderingEnabled && (
          <a href={`/pedir/${r.slug}`} style={btnStyle}>
            ...
            <span style={{ textAlign: 'center' }}>
              <span style={{ display: 'block', fontSize: '1.05rem', fontWeight: 700, lineHeight: 1.2 }}>Pedir online</span>
            </span>
            <span style={arrowStyle}>›</span>
          </a>
        )}
