...
export default function BodegaConfigPage() {
  ...
  async function toggle(next: boolean) {
    if (!restaurantId) return;
    setIngresoManual(next); setSaving(true);
    try {
      const res = await fetch("/api/panel/bodega/config", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ restaurantId, ingresoManualEnabled: next }) });
      const d = await res.json().catch(() => ({}));
      if (!res.ok) { toast.error(d.error || "No se pudo guardar"); setIngresoManual(!next); setSaving(false); return; }
      toast.success("Configuración guardada");
    } catch { toast.error("Error de conexión"); setIngresoManual(!next); }
    setSaving(false);
  }
    ...
    <div style={{ maxWidth: 640, margin: "0 auto", padding: "8px 4px 96px" }}>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, background: "var(--adm-card)", border: "1px solid var(--adm-card-border)", borderRadius: 14, padding: 16 }}>
          <div style={{ flex: 1 }}>
            <p style={{ fontFamily: F, fontSize: "0.92rem", fontWeight: 700, color: "var(--adm-text)", margin: "0 0 3px" }}>Permitir ingreso manual de stock</p>
            <p style={{ fontFamily: FB, fontSize: "0.8rem", color: "var(--adm-text2)", margin: 0, lineHeight: 1.5 }}>
              Si lo desactivas, el botón <strong>Ingreso</strong> del detalle de insumo se oculta y el stock solo entra desde el módulo <strong>Compras</strong>. Los retiros siguen disponibles.
            </p>
          </div>
          <button
            role="switch"
            aria-checked={ingresoManual}
            onClick={() => !saving && toggle(!ingresoManual)}
            style={{ width: 50, height: 30, borderRadius: 15, border: "none", cursor: saving ? "default" : "pointer", position: "relative", flexShrink: 0, background: ingresoManual ? ACCENT : "var(--adm-card-border)", transition: "background 0.2s", opacity: saving ? 0.6 : 1 }}
          >
            <span style={{ position: "absolute", top: 3, left: ingresoManual ? 23 : 3, width: 24, height: 24, borderRadius: "50%", background: "white", transition: "left 0.2s", boxShadow: "0 1px 3px rgba(0,0,0,0.3)" }} />
          </button>
        </div>
      )}
