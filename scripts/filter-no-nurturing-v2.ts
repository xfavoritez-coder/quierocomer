...
async function main() {
  ...
  console.log("Restaurants SIN nurturing, CON WhatsApp (quitando solo los que están usando activamente):\n");

  let count = 0;
  for (const r of restaurants) {
    if (nurtured.has(r.id)) continue;
    const wa = r.owner?.whatsapp || leadBySlug.get(r.slug)?.whatsapp || null;
    if (!wa || wa.length < 10) continue;
    ...
    // Solo quitar los que REALMENTE están usando (trial_usando, activo pagando)
    if (["TRIAL_USANDO", "ACTIVO"].includes(stage)) continue;

    const age = Math.floor((now.getTime() - r.createdAt.getTime()) / (24 * 60 * 60 * 1000));
    count++;
    console.log(`${count.toString().padStart(2)}. ${r.name.padEnd(35)} | ${stage.padEnd(22)} | ${wa.padEnd(16)} | ${age}d`);
  }
